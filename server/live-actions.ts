...
export async function getScheduledLiveSessions() {
  try {
    const teacherId = await requireTeacherId()
    const rows = (await sql`
      SELECT id, title, start_at, embed_url, grades, month, is_free
      FROM live_sessions
...
export async function scheduleLiveSession(input: {
  ...
  try {
    ...
    await sql`
      INSERT INTO live_sessions (id, teacher_id, title, start_at, embed_url, grades, month, is_free)
      VALUES (${id}, ${teacherId}, ${input.title}, ${input.start_at}, ${input.embed_url}, ${input.grades}, ${input.month}, ${input.is_free});
    `
